...
var SPRITE_SHEET_SRC =
  "data:image/png;base64,";
var LEFT_KEY = 37;
var RIGHT_KEY = 39;
var SHOOT_KEY = 32;
var START_KEY = 32;
// key codes: 'Space' = 32, 'x' = 88, 'enter' = 13
var PLAYER_CLIP_RECT = { x: 0, y: 203, w: 64, h: 32 };
var BUG_BOTTOM_ROW = [
  { x: 7, y: 0, w: 48, h: 49 },
  { x: 7, y: 49, w: 48, h: 51 },
];
...
var Player = SheetSprite.extend({
  ...
  shoot: function () {
    var bullet = new Bullet(this.position.x, this.position.y - this.bounds.h / 2, 1, 800);
...
var Enemy = SheetSprite.extend({
  ...
  shoot: function () {
    this.bullet = new Bullet(this.position.x, this.position.y + this.bounds.w / 2, -1, 450);
  },
  ...
  update: function (dt) {
    ...
    if (this.stepAccumulator >= this.stepDelay) {
      ...
      if (bugDirection === 1 && this.position.x > CANVAS_WIDTH - this.bounds.w / 2 - 20) {
        updateBugLogic = true;
      }
...
function preDrawImages() {
  ...
  var img = new Image();
  img.src =
    "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAUAAAAKCAYAAAB8OZQwAAAAMUlEQVQI12NkYGBgMDU1/c8ABadPn2ZkhAmcPn0azkZRBeMzMWABNBOEuQBGM2JzPACEvxQm6nrehwAAAABJRU5ErkJggg==";
  bulletImgs.push(img);
  // '1', resolution 5 x 10
  img = new Image();
  img.src =
    "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAUAAAAKCAYAAAB8OZQwAAAAJklEQVQI12NggAJTU9P/MDYTugADAwMDE7oACsDQjg5oIciIzUkAcV0IJqLp3/kAAAAASUVORK5CYII=";
  bulletImgs.push(img);
}
...
function setupBugFormation() {
  ...
  for (var i = 0, len = BUG_ROWS * BUGS_PER_COLUMN; i < len; i++) {
    ...
    bugs.push(
      new Enemy(
        clipRects,
        CANVAS_WIDTH / 2 - BUG_SQUAD_WIDTH / 2 + BUG_X_MARGIN / 2 + gridX * BUG_X_MARGIN,
  ...
  for (var i = 0, len = bullets.length; i < len; i++) {
    ...
    for (var j = 0, alen = bugs.length; j < alen; j++) {
      ...
      if (checkRectCollision(bullet.bounds, bug.bounds)) {
        ...
        particleManager.createExplosion(bug.position.x, bug.position.y, BUG_EXPLOSION_COLOR, 30, 3, 3, 4, 0.3, 50);
        player.score += 10;
      }
    }
  }
...
function resolveBulletPlayerCollisions() {
  for (var i = 0, len = bugs.length; i < len; i++) {
    var bug = bugs[i];
    if (bug.bullet !== null && checkRectCollision(bug.bullet.bounds, player.bounds)) {
      if (player.lives === 0) {
        hasGameStarted = false;
      } else {
        bug.bullet.alive = false;
        particleManager.createExplosion(player.position.x, player.position.y, SPACESHIP_COLOR, 80, 5, 5, 10, 0.005, 40);
...
function drawBottomHud() {
  ...
  ctx.drawImage(
    ...
    60,
    CANVAS_HEIGHT - 23,
    player.clipRect.w * 0.5 + 1,
    player.clipRect.h * 0.5 + 1
  );
  fillRightAlignedText("Debugging score: " + player.score, CANVAS_WIDTH, CANVAS_HEIGHT - 7.5);
}
